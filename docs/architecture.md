...
- `event`: イベント型定義(`EventEnvelope`/`EventKind`)・署名検証・チェーン連結検証(`verify_chain_link`)
- `head`: IPNS-headレコード(`IpnsRecord`)の生成・検証・シリアライズ、argmax統一規則の選択関数(`select_best`)、feed トピック名の導出
- `keystore`: Argon2 + AES-256-GCM による秘密鍵の暗号化保管
- `store`: SQLite による永続化・projection インデックス・キャッシュ(フォローリスト含む)。fold はイベント挿入時の projection 更新として実装する
- `network`: libp2p ノードのラッパー(後述)
- `util`: 横断ヘルパー(時刻・canonical DAG-CBOR コーデック・CID 計算・hex 変換)
...
エラーは各層が thiserror による型付きエラー(`KeystoreError` / `StoreError` / `SyncError` 等)を返し、`app` が `AppError` に集約する。文字列化は `commands` の IPC 境界でのみ行う。
