...
head 解決は「複数ソースから得た候補レコードのうち、署名検証OKかつ (sequence, validity) が辞書式で最大のものを選ぶ」という argmax統一規則で行う。gossipsub・DHT・フォローグラフ探索はすべて候補レコードのソースであり、優劣の階層ではない。これにより stale なレコードに引きずられず、常に最良の head へ収束する。validity を副キーにするのは、republish(§4.2)が `sequence` を変えず validity のみ更新するためで、これがないと再発行が既存レコードに対して stale と判定され反映されない。validity は署名対象なので、第三者が延命を偽装することはできない。同一 (sequence, validity) の複数候補は先に現れたものを保持する。選択とは独立に、候補集合内や保持中レコードとの間で同一 `sequence`・異なる head CID の組(equivocation)を観測したら fork として記録する([data-model.md](data-model.md) §2.1)。
...
IPNS-headレコード = { name: pubkey, value: head_cid, sequence, validity(EOL),
   ...
   選択: argmax_(sequence, validity)( 検証OKの候補群 )
```
...
- 受信側は他ノードから put されたレコードを store へ格納する前に検証する。受理条件は、署名検証に成功すること、レコードキーが payload の `name` から導出したものと一致すること(他アカウントのキーの汚染を拒否)、既に保持するレコードを (sequence, validity) の辞書式で上回ること(stale put による巻き戻しを拒否しつつ、`sequence` を変えず validity のみ更新する republish は受理する)。同一 `sequence`・異なる head CID の競合はここでは拒否のみとし、fork の記録は行わない: DHT の格納義務はフォロー外アカウントにも及ぶため、記録すると使い捨て鍵による記録の膨張の攻撃面になる。フォロー相手の fork は gossipsub / resolve / チェーン遡行の経路で検出される([data-model.md](data-model.md) §2.1)。
...
- 各ピアは手元の最良((sequence, validity) 最大)の署名済み head を返す。失効済み IPNS-headレコードや最新イベントの封筒でも、署名と sequence は検証可能なので候補として有効。
