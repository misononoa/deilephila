...
head 解決は「複数ソースから得た候補レコードのうち、署名検証OKかつ (sequence, validity) が辞書式で最大のものを選ぶ」という argmax統一規則で行う。gossipsub・DHT・フォローグラフ探索はすべて候補レコードのソースであり、優劣の階層ではない。これにより stale なレコードに引きずられず、常に最良の head へ収束する。validity を副キーにするのは、republish(§4.2)が `sequence` を変えず validity のみ更新するためで、これがないと再発行が既存レコードに対して stale と判定され反映されない。validity は署名対象なので、第三者が延命を偽装することはできない。
...
- 受信側は他ノードから put されたレコードを store へ格納する前に検証する。受理条件は、署名検証に成功すること、レコードキーが payload の `name` から導出したものと一致すること(他アカウントのキーの汚染を拒否)、既に保持するレコードを (sequence, validity) の辞書式で上回ること(stale put による巻き戻しを拒否しつつ、`sequence` を変えず validity のみ更新する republish は受理する)。
